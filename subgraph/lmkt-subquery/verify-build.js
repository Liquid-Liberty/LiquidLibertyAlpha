...
function verifyBuild(networkType) {
  ...
  // Check project.yaml (generated by SubQuery SDK)
  const yamlPath = path.join(process.cwd(), 'project.yaml');
  if (fs.existsSync(yamlPath)) {
    const yamlContent = fs.readFileSync(yamlPath, 'utf8');
    if (!yamlContent.includes(expected.treasury)) {
      console.error(`❌ ${expected.name} treasury address not found in project.yaml!`);
      process.exit(1);
    }
  }
  ...
  console.log(`   Files verified: dist/index.js, project.yaml`);
}
