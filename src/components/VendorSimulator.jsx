...
import { vendorTemplates } from '../data/mockData'; // Import our new templates
...
const VendorSimulator = () => {
    ...
    const [hasSubmitted, setHasSubmitted] = useState(false); // Simulate the one-submission rule
        ...
        return (
             <div className="mt-10 p-6 bg-green-50 border-2 border-green-200 rounded-lg shadow-lg animate-fade-in text-center">
                ...
                <div className="mt-4 inline-block bg-white p-4 rounded-lg">
                    {logoPreview && <img src={logoPreview} alt="Vendor Logo" className="h-20 w-20 rounded-full mb-3 mx-auto" />}
                ...
                <div className="mt-6">
                    <p className="text-lg font-semibold text-zinc-700">Simulated Daily Revenue to Treasury:</p>
                    <p className="text-4xl font-bold text-teal-800">${simulationResult.totalRevenue.toFixed(2)}</p>
                </div>
                <p className="text-center text-xs text-zinc-500 mt-4">
                    This data has been submitted. The dev team will settle all submissions daily, at which point you will receive your LMKT rewards.
                </p>
                 <button onClick={() => setHasSubmitted(false)} className="mt-6 text-sm text-blue-600 hover:underline">Run Another Simulation</button>
            </div>
        )
    }
        ...
        <div className="space-y-8">
            ...
            <form onSubmit={handleFormSubmit} className="space-y-6 bg-stone-100 p-8 rounded-lg shadow-md">
                ...
                <div className="text-center pt-4">
                    <button type="submit" className="bg-teal-600 text-white py-3 px-8 rounded-md hover:bg-teal-700 transition duration-300 font-bold text-lg">
