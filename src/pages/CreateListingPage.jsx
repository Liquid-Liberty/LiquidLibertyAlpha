...
const CreateListingPage = ({ listings }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // STEP 2: Request typed-data signature from backend
          ...
          <div className="flex justify-center mb-8">
            <div className="flex p-1 bg-stone-200 rounded-lg">
              <button
                ...
                className={`px-6 py-2 rounded-md font-bold ${listingType === 'item' ? 'bg-teal-800 text-white' : 'text-zinc-700'}`}
              >
                Item for Sale
              </button>
              <button
                onClick={() => setListingType('service')}
                className={`px-6 py-2 rounded-md font-bold ${listingType === 'service' ? 'bg-teal-800 text-white' : 'text-zinc-700'}`}
              >
                Service Offered
              </button>
            </div>
